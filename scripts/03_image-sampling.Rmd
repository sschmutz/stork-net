...
The goal is to randomly select 200 sample images for manual labeling which are then used to create a deep neural network which classifies if a stork is present or not.  
The ratio of stork present/absent should be around 50:50. If we're far of this, we need to sample more at a later timepoint.

*Note: Now is that later timepoint and it was decided to sample and label 400 samples instead.*
...
write_csv(files_2019_sampled, path = here("images", "2019_sampled", "files_2019_sampled.csv"))
